...
export default function CvFeedbackCard({ feedback, countryLabel }: { feedback: string; countryLabel: string }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {parsed.conventions.length > 0 && (
        <SectionCard icon={<Globe2 size={16} className="text-blue-600 dark:text-blue-400" />} title={`Convenções de ${countryLabel}`} accent="border-l-blue-400 dark:border-l-blue-700">
          <ul className="space-y-2">
      ...
      {parsed.atsKeywords.length > 0 && (
        <SectionCard icon={<Search size={16} className="text-fuchsia-600 dark:text-fuchsia-400" />} title="Palavras-chave e ATS" accent="border-l-fuchsia-400 dark:border-l-fuchsia-700">
          <ul className="space-y-2">
