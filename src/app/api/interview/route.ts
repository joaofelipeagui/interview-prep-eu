...
function evaluateInstructions(lang: string): string {
  if (lang === 'en') {
    return `<scoring_rubric>\nScore on a continuous 1.1-10.0 scale, ALWAYS with exactly one decimal place (e.g. 6.4 or 8.7), using this internationally recognized structured-interview scorecard convention (the same style used in behavioral/competency-based hiring loops), mapped to these decimal bands:\n1.1-2.9 = Does not meet expectations (vague, unstructured, no concrete evidence)\n3.0-4.9 = Below expectations (weak structure, lacking detail or metrics)\n5.0-6.9 = Partially meets expectations (reasonable structure, but incomplete or generic)\n7.0-8.4 = Meets expectations well (clear, structured, with concrete evidence)\n8.5-10.0 = Exceeds expectations (exceptional, structured, quantified, memorable)\nUse the decimal precision deliberately to differentiate answers that would otherwise tie on an integer scale — do not default to round numbers.\n</scoring_rubric>\n<instructions>\nEvaluate the candidate's answer to your interview question on THREE independent dimensions: (1) interview substance/style fit for this country, judged against the candidate's stated profession/domain in candidate_context, (2) how well the answer is STRUCTURED using the STAR framework (Situation, Task, Action, Result) — the internationally recognized standard for structuring behavioral interview answers, applicable as a communication baseline regardless of this country's additional style expectations, and (3) English language quality (grammar, vocabulary, naturalness) regardless of country — if the candidate is already a fluent/native English speaker, say so briefly instead of inventing issues. Respond entirely in English. Structure your response in this exact format with these exact headers:\n\nSCORE: [score with one decimal, e.g. 7.3] — [band label from the rubric above]\n\nSTRENGTHS:\n- [bullet points, interview substance/style]\n\nAREAS TO IMPROVE:\n- [bullet points, specific to this country's interview style; include a pace comment here if speaking_delivery was provided and notable]\n\nANSWER STRUCTURE (STAR):\nFor each component, say whether it is present, absent, or weak, with a one-line comment:\n- Situation (context): ...\n- Task (objective/responsibility): ...\n- Action (what you specifically did): ...\n- Result (measurable impact): ...\nRestructuring tip: [one concrete, specific tip on how to reorganize THIS answer into a tighter STAR flow]\n\nWHAT THE RECRUITER WANTS TO HEAR:\n[Explain, generically (not tied to the candidate's specific facts), the underlying rubric this question is actually testing for in this country's interview culture — what themes, structure, and signals a strong answer would hit. This is the hidden scoring criteria, written as coaching, not as a rewritten answer.]\n\nENGLISH — CORRECTIONS AND IMPROVEMENTS:\n- [for each notable grammar, word-choice, or unnatural-phrasing issue found in the candidate's actual words: quote the original snippet, then "→" the corrected/more natural version, then a one-line reason. If the English was already strong, say so briefly instead of inventing issues.]\n\nMODEL ANSWER:\n[a stronger rewritten version of the answer, in English, same length or shorter, structured clearly in STAR order, that would perform better with this specific interviewer style AND uses correct, natural English]\n</instructions>`
  }
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (type === 'evaluate') {
      const { countryId, professionId, customProfession, question, answer, paceWpm, fillerWordCount, lang } = payload
      const ip = clientIp(req)
      const clientId = requireClientId(req)
      const subscribed = await hasActiveSubscription(req)
      ...
      const evalLang = lang === 'en' ? 'en' : 'pt'
      const paceNote = deliveryNotes.length
        ? `\n<speaking_delivery>The candidate spoke this answer aloud (voice input). They ${deliveryNotes.join('; and ')}. Weave a brief, constructive comment about this into the areas-to-improve section if it's notable — don't invent a dedicated section for it.</speaking_delivery>`
        : ''

      const msg = await client.messages.create({
        model: 'claude-sonnet-4-6',
        max_tokens: 2000,
        system: `<role>${country.interviewerStyle}</role>\n<candidate_context>${roleContext}</candidate_context>\n<evaluation_focus>${country.evaluationFocus}</evaluation_focus>${paceNote}\n${evaluateInstructions(evalLang)}`,
        messages: [{
          role: 'user',
